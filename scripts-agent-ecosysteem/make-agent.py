...
class AgentMaker:
    ...
    def create_build_plan(self) -> Dict:
        ...
        plan = {
            ...
            "promptPath": str(self.repo_root / ".github" / "prompts" / f"{phase_prefix}.{self.agent_name}.prompt.md"),
            "runnerPath": str(self.repo_root / "scripts" / phase / f"{self.agent_name}.py"),
            "orchestrationPath": str(self.repo_root / "orchestrations" / phase / f"{self.agent_name}.orchestration.yaml"),
            "outputRoot": str(self.repo_root / "artefacten" / phase),
            "buildPlanPath": str(self.repo_root / "artefacten" / "_buildplans" / f"{agent_id.replace(':', '-')}.json"),
            "qualityGates": [
                "Nederlands B1",
