...
def generate_prompt(plan: dict, charter_content: str) -> str:
    """Generate prompt content from charter."""
    agent_name = plan['agentName']
    phase = plan['phase']
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    # Extract key sections from charter
    sections = _extract_charter_sections(charter_content)
    
    prompt = f"""# Agent Prompt: {agent_name}
...
## Rol
Je bent de **{agent_name}** agent in fase **{phase}** van de SAFe Development Value Stream.
...
{sections.get('input', '- [Zie charter voor details]')}

---

## Output
Je levert als output:
{sections.get('output', '- [Zie charter voor details]')}

---

## Werkwijze
{sections.get('werkwijze', '[Zie charter voor details]')}

---

## Kwaliteitscriteria
{chr(10).join(f'- {gate}' for gate in plan['qualityGates'])}

---

## Bevoegdheden
{sections.get('bevoegdheden', '[Zie charter voor details]')}

---

**Charter bron**: {plan['charterPath']}
"""
